/**
 * JPEG/PNG из public/assets/Ekaterina Potapova → JPEG в public/assets/ekaterina-potapova/
 * + manifest public/assets/designer-galleries/ekaterina-potapova.json
 * Запуск: npm run optimize:potapova
 */
...
const inputDir = path.join(root, 'public', 'assets', 'Ekaterina Potapova');
const outputDir = path.join(root, 'public', 'assets', 'ekaterina-potapova');
const manifestDir = path.join(root, 'public', 'assets', 'designer-galleries');
const manifestPath = path.join(manifestDir, 'ekaterina-potapova.json');
...
async function main() {
  ...
  for (const name of images.sort()) {
    ...
    console.log(`${name} → ekaterina-potapova/${outName}  (${inMb} MB → ${outMb} MB)`);
  }

  manifestNames.sort();
  await writeFile(manifestPath, JSON.stringify(manifestNames), 'utf8');
  console.log(`→ designer-galleries/ekaterina-potapova.json (${manifestNames.length} файлов)`);
  console.log('Готово.');
}
